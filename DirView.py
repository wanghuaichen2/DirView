...
def resource_path(relative_path):
    """兼容打包后的资源路径获取"""
    try:
        base_path = sys._MEIPASS  # 打包后临时目录
    except Exception:
        base_path = os.path.abspath(".")  # 脚本运行目录

    return os.path.join(base_path, relative_path)

def load_icons():
    global icon_folder, icon_file
    icon_folder = ImageTk.PhotoImage(
        Image.open(resource_path("logo/folder.png")).resize((16, 16), Image.Resampling.LANCZOS))
    icon_file = ImageTk.PhotoImage(
        Image.open(resource_path("logo/file.png")).resize((16, 16), Image.Resampling.LANCZOS))
...
btn = tk.Button(btn_frame, text="选择文件夹", command=choose_directory,
                font=("Microsoft YaHei", 10), bg="#2ecc71", fg="white",
                activebackground="#27ae60", relief=tk.FLAT, padx=10, pady=5)
...
expand_btn = tk.Button(right_btn_frame, text="展开", command=expand_all,
                       ...
                       activebackground="#2980b9", relief=tk.FLAT, padx=8, pady=4)
...
collapse_btn = tk.Button(right_btn_frame, text="合并", command=collapse_all,
                         ...
                         activebackground="#d35400", relief=tk.FLAT, padx=8, pady=4)
